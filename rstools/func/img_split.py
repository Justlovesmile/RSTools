...
from rstools.utils.poly import parse_dota_poly2
...
class ImgSplitBase():
    def __init__(self,
        ...
        print(f"[INFO] img_path: {os.path.abspath(imagepath)}, " + 
              f"label_path: {os.path.abspath(labelpath)}, " + 
              f"outpath: {os.path.abspath(outpath)}, subsize: {subsize}, " + 
              f"gap: {gap}, thresh: {thresh}, padding: {padding}, " + 
              f"num_process: {num_process}, filter_empty: {filter_empty}" + 
    ...
    def saveimagepatches(self, img, subimgname, left, up):
        ...
        outdir = os.path.join(self.outimagepath, subimgname + self.saveext)
    ...
    def savepatches(self, img, objects, subimgname, left, up, right, down):
        ...
        if self.filter_empty and HAS_OBJ==0:
            #print(f"[WARNING] Remove {outdir} due to it does not have objects.         ",end='\r')
            os.remove(outdir)
        else:
    ...
    def SplitSingle(self, name, rate, extent):
        ...
        """
        try:
            img = read_image(os.path.join(self.imagepath, name + extent))
        except Exception as e:
            print('[ERROR] Can not open image file:', name, f', due to {e}')
            return
        assert np.shape(img) != ()
        ...
        if os.path.exists(fullname):
            ...
            objects = []
            print(f"[WARNING] {fullname} not exist!")
        for obj in objects:
        ...
        while (left < weight):
            ...
            while (up < height):
                ...
                self.savepatches(img, objects, subimgname, left, up, right, down)
                if (up + self.subsize >= height):
                    break
                else:
                    up = up + self.slide
    ...
    def splitdata(self, rate):
        ...
        """
        imagelist = get_file_from_this_dir(self.imagepath, ext=self.ext)
        imagenames = [custom_basename(x) for x in imagelist if (custom_basename(x) != 'Thumbs')]
        print(f"[INFO] Scale: {rate}, images: {len(imagenames)}, start to split...")
...
class OnlyImgSplitBase():
    def __init__(self,
        ...
        self.num_process = num_process
        self.pool = Pool(num_process)
        print(f"[INFO] img_path: {os.path.abspath(srcpath)}, " + 
              f"outpath: {os.path.abspath(dstpath)}, subsize: {subsize}, " + 
              f"gap: {gap}, padding: {padding}, num_process: {num_process}")
    ...
    def saveimagepatches(self, img, subimgname, left, up):
        subimg = copy.deepcopy(img[up: (up + self.subsize), left: (left + self.subsize)])
        outdir = os.path.join(self.dstpath, subimgname + self.saveext)
        h, w, c = np.shape(subimg)
        if (self.padding):
            outimg = np.zeros((self.subsize, self.subsize, 3))
            outimg[0:h, 0:w, :] = subimg
    ...
    def SplitSingle(self, name, rate, extent):
        try:
            img = read_image(os.path.join(self.srcpath, name + extent))
        except Exception as e:
            print('[ERROR] Can not open image file:', name, f', due to {e}')
            return
        assert np.shape(img) != ()
    ...
    def splitdata(self, rate):
        imagelist = get_file_from_this_dir(self.srcpath, ext=self.ext)
        imagenames = [custom_basename(x) for x in imagelist if (custom_basename(x) != 'Thumbs')]
        print(f"[INFO] Scale: {rate}, images: {len(imagenames)}, start to split...")
        worker = partial(self.SplitSingle, rate=rate, extent=self.ext)
        self.pool.map(worker, imagenames)
